...
- R074: OZ Governor abstract base equivalence (virtual functions as explicit args)
...
- `proofs/equivalence/GovernorBase.v` — 50+ sim-level lemmas covering
  the 8-state proposal-state machine, all five mutators
  (`propose` / `castVote` / `queue` / `execute` / `cancel`), and
  the validity-preservation chain (see R074).
