...
/* ----- S27.1 HEADLINE: castVote on optimistic proposal credits
   ...
   Solver-budget note (per WISDOM C022): this scenario rule resolves
   in ~17 minutes wall-clock on a local Certora build. The dominant
   cost is the post-_castVote run of _tallyUpdated -> state() which
   re-traverses the threshold-vs-tally math. The rule still
   discharges within budget; bumping `optimistic_loop`/`loop_iter`
   is not required.
*/
rule castVoteOptimisticReadsOptimisticVotes {
    ...
    // Tame the search: keep weights in a sensible range so the
    // accumulator does not overflow (uint256 already protects, but
    // smaller bound lets the solver finish faster).
    require optWeight < 1000000000000000000000000000;  // 1e27
    ...
    require to_mathint(againstBefore) + to_mathint(optWeight) <= max_uint256;
...
/* ----- S27.2 SANITY: castVote precondition is satisfiable.
   VIOLATED here is healthy (WISDOM C002). */
